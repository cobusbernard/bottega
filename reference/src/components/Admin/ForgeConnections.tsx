...
function ForgeConnections() {
  const [connections, setConnections] = useState<ForgeConnectionRow[]>([]);
  ...
  const handleToggle = async (conn: ForgeConnectionRow) => {
    try {
  ...
  const handleDelete = async (conn: ForgeConnectionRow) => {
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {connections.map((conn) => (
                <tr key={conn.id} className="border-b border-border/50 hover:bg-accent/50">
